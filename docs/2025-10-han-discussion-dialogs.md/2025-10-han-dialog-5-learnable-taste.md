...
1. **Aesthetics Reward Model (R_\theta)** (tiny MLP or logistic head; 10–100k params).
   ...
   * Inputs: a feature vector (f(code)) (below) + critic committee votes.
...
### Feature extractor (f(code)) (all automated, frozen tools)
...
1. **Metrics first**: implement (f(code)) with:
...
R_theta(code) = sigmoid(theta · f(code))  # or raw score
```
